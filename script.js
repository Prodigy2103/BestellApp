...
function renderMenu() {
    ...
    if (itemRef) { // Überprüft, ob das Element itemRef (also das Element mit der ID "cartDish") gefunden wurde.
        itemRef.innerHTML = ""; // Leert den Inhalt des gefundenen HTML-Elements.

        if (
            typeof menuItemsArray !== "undefined" &&
            menuItemsArray.length > 0) { // Prüft, ob das menuItemsArray Elemente enthält nicht leer ist.
            for (let i = 0; i < menuItemsArray.length; i++) { // Startet eine Schleife, die durch jedes Element im menuItemsArray läuft.
                const item = menuItemsArray[i]; // Holt das aktuelle Element des Arrays in der Schleife und speichert es in item.
                itemRef.innerHTML += getMenuItem(i, item.title, item.desc, formatPrice(item.price), item.img); // Fügt den HTML-Code, der von der Funktion getMenuItem generiert wird, zum itemRef-Element hinzu. Dabei werden Index, Titel, Beschreibung, formatierter Preis und Bild des Menüpunkts übergeben
            }
        }
    }
}
...
function renderCart() {
    ...
    const allOrderItemsHtml = (typeof orderArray !== "undefined" && orderArray.length > 0) ? //Beginnt eine Operation, die prüft, ob orderArray definiert ist und Elemente enthält.
        orderArray.map((entry, j) => { //Wenn orderArray existiert und nicht leer ist, wird es durchlaufen und jedes Element wird einer Funktion übergeben.
    ...
    if (orderRef) { // Prüft, ob das HTML-Element orderRef (also das Element mit der ID "order") gefunden wurde
        orderRef.innerHTML = getOrderContainerTemplate(allOrderItemsHtml, subtotal, total); //Setzt den inneren HTML-Inhalt des orderRef-Elements auf den von getOrderContainerTemplate generierten HTML-Code, der die Bestellelemente, die Zwischensumme und den Gesamtbetrag enthält.
    }
}
...
function renderCartView() {
    ...
    if (orderRef) {
        orderRef.innerHTML = getOrderContainerTemplate(allOrderItemsHtml, subtotal, total);
    }
}

function addToCart(menuItemIndex) {
    if (!menuItemsArray?.[menuItemIndex]) { //Prüft, ob das menuItemsArray existiert und ob ein Element am übergebenen menuItemIndex vorhanden ist. Das ?. (optional chaining) stellt sicher, dass kein Fehler auftritt, wenn menuItemsArray selbst undefiniert oder null ist.
        console.error("Menüelement nicht gefunden oder menuItemsArray nicht initialisiert."); //Wenn die obige Bedingung wahr ist (d.h., das Menüelement wurde nicht gefunden), wird eine Fehlermeldung in der Konsole ausgegeben.
        return; //Beendet die Funktion vorzeitig, wenn ein Fehler auftritt, um zu verhindern, dass weiterer Code ausgeführt wird.
    }
    orderArray = orderArray || []; //Stellt sicher, dass orderArray (wahrscheinlich das Array, das die Artikel im Warenkorb speichert) initialisiert ist. Wenn orderArray noch nicht existiert (also undefined oder null ist), wird es als leeres Array [] zugewiesen.

    const itemToAdd = menuItemsArray[menuItemIndex]; //Holt das spezifische Menüelement aus dem menuItemsArray anhand des menuItemIndex und speichert es in der Konstante itemToAdd.
    const existingItem = orderArray.find(entry => entry.item.index === itemToAdd.index); //Sucht im orderArray (dem Warenkorb), ob der Artikel, der hinzugefügt werden soll (itemToAdd), bereits existiert. Es wird überprüft, ob der index des Artikels im Warenkorb mit dem index des hinzuzufügenden Artikels übereinstimmt

    if (existingItem) { //Wenn existingItem existiert (d.h., der Artikel ist bereits im Warenkorb).
        existingItem.quantity++; //Erhöht die quantity (Menge) des bereits im Warenkorb befindlichen Artikels um eins.
    } else {
        orderArray.push({ item: { ...itemToAdd, index: menuItemIndex }, quantity: 1 }); //Fügt ein neues Objekt zum orderArray hinzu. Dieses Objekt enthält:
    }
    renderCart();
    renderCartView();
}
...
function addOneProduct(cartItemIndex) {
    if ( //Beginnt eine Bedingungsprüfung, um sicherzustellen, dass die Operation sicher durchgeführt werden kann.
...
function removeOneProduct(cartItemIndex) {
    if (
        ...
        if (orderArray[cartItemIndex].quantity <= 0) { // Überprüft, ob die Menge des Artikels nach der Verringerung null oder kleiner ist.
...
function setupCartInteractionListener(containerId) {
    ...
    if (container) { // Prüft, ob das container-Element tatsächlich im Dokument gefunden wurde.
        container.addEventListener("click", function (event) { //Fügt dem container-Element einen Event-Listener hinzu. Dieser Listener wartet auf click-Ereignisse. Wenn ein Klick innerhalb des Containers stattfindet, wird die anonyme Funktion ausgeführt, wobei das event-Objekt übergeben wird.
...
function init() {
    ...
    if (orderButton) { //Überprüft, ob das orderButton-Element im HTML gefunden wurde.
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (cartToggleButton && cartContainer) { //Überprüft, ob beide Elemente – der Umschalter und der Container – erfolgreich im Dokument gefunden wurden. Dies verhindert Fehler, falls eines der Elemente fehlt.
        cartToggleButton.addEventListener("click", () => { //Fügt dem cartToggleButton einen Event-Listener hinzu. Bei jedem Klick auf diesen Button wird die anonyme Pfeilfunktion (() => { ... }) ausgeführt.
